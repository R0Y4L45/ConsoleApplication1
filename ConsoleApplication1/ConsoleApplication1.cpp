...
bool sort_Name(Debtor Debtor1, Debtor Debtor2) {
	return (Debtor1.get_FullName() < Debtor2.get_FullName());
};
...
int main() {
	...
	//for_each(debtors.begin(), debtors.end(), [](Debtor d)
	//	{
	//		string email = d.get_Email(), temp, rhyta = "rhyta.com", dayrep = "dayrep.com";
	//		size_t index = email.rfind('@');

	//		temp.resize(email.size() - (index + 1));

	//		for (size_t i = index + 1, z = 0; i < email.size(); i++)
	//			temp[z++] = email[i];

	//		if (temp == rhyta || temp == dayrep)
	//			cout << email << endl;
	//	});
	...
	// 3) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag
		...
		//if ((2022 - debtor.get_Birthday().get_year()) >= 26 && (2022 - debtor.get_Birthday().get_year()) < 36)
		//	debtor.print();
		//	});
	...
	// 5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
		 ...
		 // for_each(debtors.begin(), debtors.end(), [](Debtor debtor) {
		 //     if (debtor.get_FullName().length() < 19 && debtor.get_Phone().length() < 2)
		 //         debtor.print();
		 //     });
	...
	//  7) Qishda anadan olan borclulari cixardmaq
		...
		//for_each(debtors.begin(), debtors.end(), [](Debtor debtor) {
		//    if (debtor.get_Birthday().get_month() == "December" || debtor.get_Birthday().get_month() == "January")
		//        debtor.print();
		//    });
	...
	// 8) Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek
	 ...
	 // 9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
		 ...
		 //for_each(debtors.begin(), debtors.end(), [&](Debtor debtor) {
		 //     int count = 0;
		 //     for (size_t i = 0; i < debtor.get_Phone().size(); i++)
		 //     {
		 //         if (debtor.get_Phone()[i] != '8') {
		 //             cout << debtor.get_FullName() << endl << 2022 - debtor.get_Birthday().get_year() << endl << debtor.get_Debt() << endl;
		 //             return;
		 //         }
		 //         else
		 //             return;
	...
	// 11)  Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin siyahisin cixarmaq ve onlari elifba sirasina gore
	...
	//		string surname = d.getSurname(), name = d.getName(), temp, temp1;
	...
	//		{
	//			for (size_t z = 0; z < temp1.length(); z++)
	//			{
	//				if (temp[i] == temp1[z])
	//				{
	//					c++;
	...
	// 16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
		 ...
		 //for_each(debtors.begin(), debtors.end(), [](Debtor debtor) {
		 //    if (debtor.get_Birthday().get_year() >= 1941 && debtor.get_Birthday().get_year() < 1945)
		 //        debtor.print();
		 //});
